#!/usr/bin/env python3
"""Reveal and score the adaptive 500M/10-TPP Jiang endpoint."""
...
def main() -> None:
    ...
    if preregistration.get("status") != "preregistered":
        raise ValueError("500M preregistration is invalid")
    records = list(shard.get("records", ()))
    if shard.get("status") != "completed" or len(records) != 1:
        raise ValueError("500M shard is incomplete or ambiguous")
    record = records[0]
    ...
    gates = {
        ...
        "task_matches_preregistration": len(shard.get("assigned_tasks", ())) == 1
        and shard["assigned_tasks"][0]["task_id"] == preregistration["task_id"],
        "geometry_matches_preregistration": int(record["parameter_count"])
        ...
        "optimizer_groups_complete_and_disjoint": record["metadata"][
