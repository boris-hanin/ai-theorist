...
jiang_root="$suite_root/jiang-chizat-fixed-budget"
completep_root="$suite_root/completep-fixed-budget"
...
if (( gpu_count != 8 )); then
  echo "fixed-budget suite requires exactly eight visible GPUs; found $gpu_count" >&2
  exit 1
fi
...
current_stage="binding-immutable-corpus"
echo "$current_stage" > "$suite_root/stage"
"$cli" forecast-bind \
  configs/autoscaler/jiang_mistral_fixed_budget_100m_rho32_adamw.json \
  "$manifest" --output "$jiang_root/bound-config.json" \
  > "$jiang_root/bind-summary.json"
"$cli" forecast-bind \
  configs/autoscaler/completep_mistral_fixed_budget_100m_adamw.json \
  "$manifest" --output "$completep_root/bound-config.json" \
...
current_stage="qualifying-parameter-groups-and-runtime"
echo "$current_stage" > "$suite_root/stage"
CUDA_VISIBLE_DEVICES=0 "$python" scripts/qualify_fixed_budget_runtime.py \
  "$jiang_root/bound-config.json" "$completep_root/bound-config.json" \
  --output "$suite_root/runtime-qualification.json" \
  > "$suite_root/runtime-qualification.stdout.json"

current_stage="preregistering-fixed-budget-pair"
echo "$current_stage" > "$suite_root/stage"
"$python" scripts/preregister_fixed_budget_100m_pair.py \
  "$jiang_root/bound-config.json" "$completep_root/bound-config.json" \
  "$suite_root/runtime-qualification.json" \
...
current_stage="tuning-reference-hyperparameters"
echo "$current_stage" > "$suite_root/stage"
...
current_stage="selecting-valid-reference-optima"
echo "$current_stage" > "$suite_root/stage"
...
current_stage="aggregating-scaling-laws"
echo "$current_stage" > "$suite_root/stage"
...
"$cli" forecast-aggregate "$jiang_root/bound-config.json" \
  ...
  --output "$jiang_root/aggregate" > "$jiang_root/aggregate.stdout.json"
"$cli" forecast-aggregate "$completep_root/bound-config.json" \
  "${completep_tune_cache_args[@]}" "${completep_ladder_cache_args[@]}" \
...
current_stage="evaluating-hidden-rungs"
echo "$current_stage" > "$suite_root/stage"
"$python" scripts/evaluate_fixed_budget_100m_pair.py \
