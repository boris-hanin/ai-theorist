...
def test_normalized_eta_conversions_are_parameterization_specific_and_invertible():
    ...
    # Chizat is group-wise in L/M/D.  The scalar field preserves eta; the
    # authoritative raw rates are reported per semantic parameter group.
    assert chizat_raw == pytest.approx(3.0)
    assert normalized_eta_from_raw_learning_rate(
