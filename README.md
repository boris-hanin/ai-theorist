...
- `src/ai_theorist/autoscaler/`: the first product slice: strict residual-MLP,
  Chizat-particle, and sparse-MoE schemas; real SGD, Adam, and semantically
  routed Chizat Muon training; three versioned task families; one-dimensional
...
- `apps/web/`: the drag-and-drop Autoscaler workbench.  Its canvas intentionally
  compiles only `Embed -> repeated {pre-norm MLP, Chizat particle, or top-k
  MoE} residual block -> Unembed`; both boundary maps are trained.
...
ai-theorist-autoscale sample-spec /tmp/muon.json --optimizer muon --architecture chizat_mlp --quick
ai-theorist-autoscale matrix-plan configs/autoscaler/a100_chizat_optimizer_dataset_matrix.json
