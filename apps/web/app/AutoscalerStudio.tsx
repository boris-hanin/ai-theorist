...
type BlockType = "pre_norm_mlp" | "pre_norm_moe";
...
export function AutoscalerStudio() {
  ...
  const spec = useMemo<StudySpec>(() => ({
    schema_version: 2,
    name: `${optimizer}-${blockType === "pre_norm_moe" ? "moe" : "mlp"}-fixed-horizon`,
    architecture: {
      block_type: blockType,
      activation,
      input_dim: 16,
    ...
    seeds: [11, 29],
    holdout_count: 1,
  }), [activation, activeExperts, blockType, numExperts, optimizer, datasetSize, steps, scales, learningRates]);
  ...
  function chooseBlock(next: BlockType) {
    if (studyLocked) return;
    if (next !== blockType) setScales(next === "pre_norm_moe" ? MOE_SCALES : INITIAL_SCALES);
    setBlockType(next);
    if (next === "pre_norm_moe") setOptimizer("adam");
    ...
    <main>
      ...
      <section className="workspace section-shell" id="architecture">
        ...
        <div className="builder-grid">
          <aside className="panel palette-panel">
            ...
            {(["pre_norm_mlp", "pre_norm_moe"] as BlockType[]).map((item) => (
              <button
              ...
              >
                ...
                <span>
                  <strong>{item === "pre_norm_moe" ? "Sparse MoE" : "Pre-norm MLP"}</strong>
                  <small>{item === "pre_norm_moe" ? "Top-k expert routing" : "Dense residual cell"}</small>
                </span>
                <b>{blockType === item ? "In use" : "Add"}</b>
              </button>
            ))}
            <div className="coming-soon"><span>Later</span><p>Attention, convolution, AdamW, Muon, and MoE with SGD remain outside this validated slice.</p></div>
          </aside>
            ...
            <div className="model-graph">
              ...
              <button
                ...
                onDrop={(event) => {
                  ...
                  if (item === "pre_norm_mlp" || item === "pre_norm_moe") chooseBlock(item);
                }}
              >
                <span className="node-index">02</span>
                <span><small>Repeatable cell</small><strong>Residual stack</strong><em>{blockType === "pre_norm_moe" ? `Sparse MoE · ${numExperts} experts · top ${activeExperts}` : "Pre-norm MLP"} · {activation.toUpperCase()}</em></span>
      ...
      <section className="study-section section-shell" id="study">
        <div className="section-heading">
          ...
          <p>{blockType === "pre_norm_moe" ? "Depth L, expert width M, and embedding D follow the validated LM/D path. Expert sparsity stays fixed." : "Only width and repeat count grow."} The dataset and training horizon remain fixed.</p>
        </div>
        ...
        <div className="study-grid">
          <div className="panel scale-panel">
            ...
            <div className="scale-table" role="table" aria-label="Model scale ladder">
              <div className={`scale-row table-head ${blockType === "pre_norm_moe" ? "moe-scale-row" : ""}`} role="row"><span>Level</span><span>Width D</span><span>{blockType === "pre_norm_moe" ? "Depth L" : "Repeats R"}</span>{blockType === "pre_norm_moe" && <span>Expert M</span>}<span>Parameters</span><span>Role</span></div>
              {scales.map((scale, index) => (
                <div className={`scale-row ${blockType === "pre_norm_moe" ? "moe-scale-row" : ""}`} role="row" key={scale.name}>
