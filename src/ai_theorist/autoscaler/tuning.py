...
def raw_learning_rate_from_normalized_eta(
    ...
    *,
    ...
    if parameterization == CHIZAT_MEAN_FIELD:
        # Chizat is intrinsically group-wise.  This scalar is the normalized
        # coordinate retained for compatibility; raw group rates are
        # authoritative and reported separately.
        return eta
...
def normalized_eta_from_raw_learning_rate(
    ...
    *,
    ...
    if optimizer_name not in {"sgd", "adam", "muon"}:
        raise ValueError(f"Unsupported optimizer transfer rule: {optimizer_name}")
    if optimizer_name in {"adam", "muon"}:
        return raw
    if parameterization == STANDARD_RESIDUAL_MLP:
...
def optimizer_group_learning_rates_from_normalized_eta(
    ...
    *,
    ...
    if parameterization == CHIZAT_MEAN_FIELD:
        if isinstance(particle_width, bool) or not isinstance(particle_width, int) or particle_width <= 0:
            raise ValueError("particle_width must be a positive integer")
        eta = _positive_finite(normalized_eta, "normalized_eta")
        D, L, M = width, depth, particle_width
        if optimizer_name == "sgd":
            ...
            allowed_rules = {"declared", "wrong_global_LMD", "single_global_control"}
            if rule == "wrong_global_LMD":
                rates["U"] = rates["W"]
            elif rule == "single_global_control":
                rates = {name: eta for name in rates}
        elif optimizer_name in {"adam", "muon"}:
            rates = {
                "embed": eta,
                "U": eta,
                "W": math.sqrt(D) * eta,
            ...
            allowed_rules = {
                "declared", "wrong_W_D", "wrong_sgd_LMD",
                "wrong_constant_unembed", "single_global_control",
            }
            if rule == "wrong_W_D":
                rates["W"] = D * eta
            ...
            raise ValueError(f"Unsupported Chizat optimizer: {optimizer_name}")
        if rule not in allowed_rules:
            raise ValueError(f"unsupported {optimizer_name} Chizat rate rule: {rule}")
        return {name: float(rate) for name, rate in rates.items()}
    if rule != "declared":
        raise ValueError(f"rate rule {rule} is only supported for Chizat")
    if parameterization != MOE_TABLE1_ADAM:
        return {"all": base}
...
def transfer_rule_name(
    ...
    if parameterization == CHIZAT_MEAN_FIELD:
        if optimizer_name not in {"sgd", "adam", "muon"}:
            raise ValueError(f"Unsupported Chizat optimizer: {optimizer_name}")
        return f"chizat_{optimizer_name}_semantic_group_rates_from_normalized_eta"
    if parameterization == MOE_TABLE1_ADAM:
        if optimizer_name != "adam":
            raise ValueError("moe_table1_adam does not certify SGD")
...
def parameterization_for_block_type(block_type: str) -> str:
    if block_type == "pre_norm_moe":
        return MOE_TABLE1_ADAM
    if block_type == "chizat_mlp":
        return CHIZAT_MEAN_FIELD
    if block_type == "pre_norm_mlp":
        return STANDARD_RESIDUAL_MLP
