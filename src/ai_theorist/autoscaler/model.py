...
class ChizatResidualMLP(nn.Module):
    """Bias-free trained Embed -> Chizat particles -> Unembed network."""

    def __init__(self, architecture: ArchitectureTemplate, scale: ScaleLevel) -> None:
        super().__init__()
        if scale.particle_width is None:
            raise ValueError("chizat_mlp requires particle_width")
        self.width = scale.width
        self.particle_width = scale.particle_width
        self.depth = scale.repeats
        self.residual_multiplier = architecture.residual_multiplier
        self.embed = nn.Parameter(torch.empty(architecture.input_dim, self.width))
        self.U = nn.ParameterList(
            nn.Parameter(torch.empty(self.width, self.particle_width))
            for _ in range(self.depth)
        )
        self.W = nn.ParameterList(
            nn.Parameter(torch.empty(self.particle_width, self.width))
            for _ in range(self.depth)
        )
        self.unembed = nn.Parameter(torch.empty(self.width, architecture.output_dim))
        self.reset_parameters()

    def reset_parameters(self) -> None:
        nn.init.normal_(self.embed, mean=0.0, std=1.0 / math.sqrt(self.embed.shape[0]))
        for U, W in zip(self.U, self.W):
            nn.init.normal_(U, mean=0.0, std=1.0 / math.sqrt(self.width))
            nn.init.normal_(W, mean=0.0, std=1.0)
        nn.init.normal_(self.unembed, mean=0.0, std=1.0 / self.width)

    def forward(self, x: Tensor) -> Tensor:
        h = x @ self.embed
        coefficient = self.residual_multiplier / (self.depth * self.particle_width)
        for U, W in zip(self.U, self.W):
            h = h + coefficient * (torch.tanh(h @ U) @ W)
        return h @ self.unembed

    def semantic_parameter_roles(self) -> Dict[str, List[nn.Parameter]]:
        return {
            "embed": [self.embed],
            "U": list(self.U),
            "W": list(self.W),
    ...
    def optimizer_parameter_groups(self, rates: Dict[str, float]) -> List[Dict[str, object]]:
        roles = self.semantic_parameter_roles()
        if set(rates) != set(roles):
            raise ValueError("Chizat optimizer rates must cover embed, U, W, and unembed")
        return [
            {"name": role, "params": parameters, "lr": float(rates[role])}
            for role, parameters in roles.items()
        ]

    def routing_loads(self) -> None:
        return None

    def update_router_balance(self, loads=None) -> None:
        del loads


def build_model(architecture: ArchitectureTemplate, scale: ScaleLevel) -> nn.Module:
    if architecture.block_type == "chizat_mlp":
        return ChizatResidualMLP(architecture, scale)
    return ResidualMLP(architecture, scale)


def dataset_fingerprint(architecture: ArchitectureTemplate, dataset: DatasetSpec) -> str:
    payload = {
        "architecture_io": [architecture.input_dim, architecture.output_dim],
...
def make_teacher_dataset(
    ...
    *,
    ...
    """Create one versioned deterministic task shared by every model scale."""
    ...
    if dataset.kind == "linear":
        y = x @ w_linear
    elif dataset.kind == "tanh_teacher":
        y = torch.tanh(1.3 * (x @ w_linear))
    elif dataset.kind == "sinusoid_quadratic":
        y = torch.sin(1.3 * (x @ w_linear)) + 0.35 * ((x.square() - 1.0) @ w_quad)
    else:  # Defensive: DatasetSpec rejects this before model construction.
