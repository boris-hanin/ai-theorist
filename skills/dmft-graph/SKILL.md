...
- **The paper's §2.4 says `sigma_{L+1} = 1` for Adam. That is a typo** — its own
  ...
  §2.4 gets a decoder rate `sqrt(D)` too large. It is a real algebraic mismatch,
  but round 011's power control drifted only `0.110` decades and did **not** meet
  the registered `0.3`-decade transfer-failure bar.
...
- **No successful end-to-end transfer validation.** P3 failed, the Q/K and
  unnormalised-message controls did not bite as registered, and the final
  signGD heads rerun was interrupted.
