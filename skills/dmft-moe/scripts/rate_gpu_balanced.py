...
def ens(C, S, X, Y, steps=8, eta=0.3, E=16, a=4, sd0=0, eta_bias=1.0):
    """Return (S,P) outputs for S independent draws at budget C."""
    L, D, M = shape(C, a)
    cL = 1.0/(L*M); d0 = X.shape[1]; kappa = a/E
    outs = []
    ...
    while done < S:
        ...
        def fwd():
            ...
            for l in range(L):
                ...
                with torch.no_grad():
                    ...
                    loads[l]=m.mean(1)                  # (S,E) batch load per seed
        ...
        for _ in range(steps):
            ...
            with torch.no_grad():
                ...
                # 2601.20205 Eq.(2): auxiliary-loss-free load balancing. NOT a
                # gradient step -- the biases get no gradient and are driven by
                # the measured batch load alone.
                for l in range(L):
                    if loads[l] is not None: b[l]-=eta_bias*(loads[l]-kappa)
        with torch.no_grad():
...
EB=float(sys.argv[2]) if len(sys.argv)>2 else 1.0
print('eta_bias =',EB,flush=True)
F={}
for C in Cs:
    t0=time.time(); L,D,M=shape(C)
    F[C]=ens(C,S,X,Y,sd0=int(math.log10(C)*1000),eta_bias=EB)
...
json.dump({'C':[r[0] for r in res],'Ediff':[r[1] for r in res],'slope':sl,'slope_tail':slt},
          open('/tmp/rate_out_eb%g.json'%EB,'w'))
